...
window.checkProbTable = function() {
    ...
    if (allCorrect) {
        document.getElementById('pt-check-btn').disabled = true;
        showPtFlash("Correct!", "success");
        
        ptRound++;
        setTimeout(() => {
            if (ptRound > totalPtRounds) finishPtGame();
            else startPtRound();
        }, 1200);
    } else {
        showPtFlash("Check your math.", "error");
    }
};

function finishPtGame() {
    ...
    qContent.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:400px; animation: fadeIn 0.5s;">
            ...
            <h2 style="color:#1e293b; margin:10px 0;">Probability Mastered!</h2>
            <p style="color:#64748b; font-size:16px;">Skills updated.</p>
        </div>
    `;

    const currentMain = window.userMastery?.['ProbTable'] || 0;
    const newMain = Math.max(0, Math.min(10, currentMain + 1));
    window.userMastery['ProbTable'] = newMain;

    if (window.supabaseClient && window.currentUser) {
        const hour = sessionStorage.getItem('target_hour') || "00";
        window.supabaseClient.from('assignment7')
            .update({ 'ProbTable': newMain })
            .eq('userName', window.currentUser)
            .eq('hour', hour)
            .then(({ error }) => { if (error) console.error("[ProbTable] Update Error:", error); });
    }
...
function showPtFlash(msg, type) {
    const overlay = document.getElementById('pt-flash');
    if (!overlay) return;
